...
numInitPts      = numPts;                               %number of points to posibly use as initilization points
...
while numInitPts
    ...
    while 1     %loop untill convergence
        ...
        %*** plot stuff ****
        if plotFlag
            figure(5678910),clf,hold on
            if numDim == 2
                plot(app.UIAxes, dataPts(1,:),dataPts(2,:),'.')
                plot(app.UIAxes,dataPts(1,myMembers),dataPts(2,myMembers),'ys')
                plot(app.UIAxes,myMean(1),myMean(2),'go')
                plot(app.UIAxes,myOldMean(1),myOldMean(2),'rd')
                pause
            end
        end
